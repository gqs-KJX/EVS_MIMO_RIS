...
addpath('./tensor_toolbox_3.1')
system_parameter;

trials = 100;         % number of Monte-Carlo trials
SNRs = [0,5,10,15,20,25,30]; % signal-to-noise ratio
...
%     for j = 1:trials
      ...
      [H_up_noisy,H_down_noisy,sigma_2] = add_noise(H_up,H_down,SNR);
%       size(H_up_noisy)
%       size(H_down_noisy)
...
%           [A0,B0,C0,~] = C_CPD(H_up_noisy_4D,P,maxiter,tol);
          [~, A0, B0, C0, ~, ~] = C_CPD1(H_up_noisy_4D,P, sigma_2);
...
%           [~, A_init, B_init, C_init, ~, ~] = C_CPD1(H_up_noisy_4D,P, sigma_2);
