...
if Mr>1
    ...
    for i1 = 1:2
        for i2 = 1:2
            i3 = i3 + 1;
            Hbreve(:,i3) = reshape(H((i1-1)*Mr+1:i1*Mr,(i2-1)*NN+1:i2*NN),Mr*NN, ); %H4，生成一个矩阵，M*N×4的矩阵，相当于IMDF的向量化
        end
    end
    
    X = reshape(Hbreve, [Mr,left,right,size(Hbreve,2)]);%重塑成四阶张量
    [N1, N2, N3, nSnap] = size(X);
    [K1,L1,K2,L2,K3,L3] = findPairs_3d(N1,N2,N3,nSnap);%根据论文中的LEMMA2中的条件自动搜索最优的K1，K2，K3
            
    Ztot = [];
    for i1 = 1:nSnap
        X1 = squeeze(X(:,:,:,i1));
        X1s = IMDFS3(X1, N1, N2, N3, K1, K2, K3);%根据折叠尺寸Ki得到K1*K2*K3 x L1*L2*L3的矩阵，如IMDF所示
        [LL1,LL2]=size(X1s);
        x1s = X1s(:);
