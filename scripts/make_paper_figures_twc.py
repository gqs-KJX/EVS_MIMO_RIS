...
CAMPAIGNS = {
    "v3": {
        ...
        "benchmark": [RES / "paper_v3" / "benchmark_as_published_960_5"],
        "benchmark_clock": [RES / "paper_v3" / "benchmark_as_published_960_5"],
        ...
        "benchmark_runtime": [RES / "paper_v3" / "benchmark_runtime30_cpu_as_published_v2"],
        # Host-matched to "benchmark_runtime" above: both serialized cost
        # suites must be profiled on one machine or their totals are not
        # comparable.  The _v2 copy of this suite is numerically identical
        # (9 significant digits on every accuracy metric) but was profiled on
...
def rate_ticks(floor, top):
    """Decade ticks for a log rate axis, extended only as far as the data."""
    ticks = [floor, 1.0, 10.0]
    if top > 10.0:
        ticks.append(100.0)
    return ticks


def rate_ticklabels(floor, top):
    labels = ["$0$", "1", "10"]
    if top > 10.0:
        labels.append("100")
    return labels
...
def fig_ablation(out: pathlib.Path) -> None:
    ...
    order = [
        ("old_stage1_ccop", "R3", "old Phase I (R3)", C["r3"], "^"),
    ...
    b.set_ylabel(r"Phase I basin acquisition [\%]")
    b.set_ylim(0, 108)
...
def fig_receiver(out: pathlib.Path) -> None:
    ...
    b.set_title("(b) Efficiency ratio, 95% bootstrap interval", pad=3)
    grid(b)
...
def fig_benchmark(out: pathlib.Path) -> None:
    ...
    # Highest rate actually drawn (curve or interval cap).  A hardcoded top
    # silently clipped a comparator once a baseline reached 100%, which is
    # precisely the value the panel has to show.
    pos_top = 0.0
    for v, lab, col, mk, ls in series:
        x, y = curve(
            B,
        ...
        pos_top = max(pos_top, float(np.nanmax(pct_)))
        ...
        pos_top = max(pos_top, float(np.nanmax(yh)))
    ...
    c.set_ylim(floor * 0.75, max(90.0, 1.30 * pos_top))
    c.set_yticks(rate_ticks(floor, pos_top))
    c.set_yticklabels(rate_ticklabels(floor, pos_top))
    ...
    for v, lab, col, mk, ls in series:
        ...
        pct_ = np.clip(100 * y, floor, None)
        clk_top = max(clk_top, float(np.nanmax(pct_)))
        ...
        yh = np.clip(100 * hi[k], floor, None)
        ym = np.clip(100 * y[k], floor, None)
        clk_top = max(clk_top, float(np.nanmax(yh)))
    ...
    f.set_ylim(floor * 0.75, max(30.0, 1.30 * clk_top))
    f.set_yticks(rate_ticks(floor, clk_top))
    f.set_yticklabels(rate_ticklabels(floor, clk_top))
...
def fig_robustness(out: pathlib.Path) -> None:
    ...
    for v, lab, col, mk, ls in [("mksc_ccop", "full estimator (Phase I + CCOP-JVP)", C["proposed"], "o", "-"),
                                ("stage1_only", "Phase I output only", C["r2"], "s", "--")]:
...
def fig_generalization(out: pathlib.Path) -> None:
    ...
    for ax, (m, lab) in zip(axs, [("scalar", "(a) Scalar"),
                                  ("dual_pol", "(b) Dual-polarized"),
                                  ("full_6d", "(c) Full EVS")]):
        spread = 0.0
...
def fig_cost(out: pathlib.Path) -> None:
    ...
    for r in T1:
        ...
        label_cfg = {
            ...
            # OMP is near the right boundary and, once its marker sits close
            # to the top of the y-range, an upward label collides with the
            # panel title -- so its label goes below instead.
            "nf_ris_groupomp_localgrid_wls": {
                "xytext": (-8, -8),
                "ha": "right",
                "va": "top",
